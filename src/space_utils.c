...
/*	double a = sdb[x].move.ratio; */
    double a = sdb[x].engine.warp_hard_cruise;
	...
	if (sdb[x].status.tractoring) {
		a *= (sdb[x].structure.displacement + 0.1) /
		  (sdb[x].structure.displacement + sdb[sdb[x].status.tractoring].structure.displacement + 0.1);
	} else if (sdb[x].status.tractored)
		a *= (sdb[x].structure.displacement + 0.1) /
		  (sdb[x].structure.displacement + sdb[sdb[x].status.tractored].structure.displacement + 0.1);

	a = a * p;  /* This section changed by Anya STDW for setting up hard-limit cruise warp */
	if (a < 1.0) {
		return 0.0;
	} else
		return a;
...
/*  The above section replaced with below for hard-limit cruise impulse - Anya STDW */

    double a = sdb[x].engine.impulse_hard_cruise;
    p /= sdb[x].aux.gw;
...
/*	if (a <= 0.0)
	...
	if (p <= 0.0)
		return 0.0;
*/
/* The above is redundant for the changes to hard-limit cruise impulse - Anya STDW */
	...
	if (sdb[x].status.tractoring) {
		...
		a *= (sdb[x].structure.displacement + 0.1) /
		  (sdb[x].structure.displacement  + sdb[sdb[x].status.tractored].structure.displacement + 0.1);
/* The above swapped the divisor and the quotient for hard-limit cruise impulse - Anya STDW */
	a = a * p;
	if (a <= 0.0) {
		return 0.0;
	} else if (a >= 1.0) {
...
/*  Line replaced with the one below to use hard cruise numbers - Anya STDW */
    a = sdb[x].engine.warp_hard_cruise;
	if (a < 1.0)
		return 0.0;
...
/*  Line replaced with the ne below to use hard cruise numbers - Anya STDW */
    a = sdb[x].engine.impulse_hard_cruise;
	if (a <= 0.0)
		return 0.0;
