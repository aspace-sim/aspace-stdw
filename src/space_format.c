...
/* ------------- Anyasha - STDW ---------- Added by Sion 12/10/07 --------- */

char *format_l_DetCoords (int x)
{
	static char buffer[120];
	
	snprintf(buffer, sizeof(buffer), "%s%16.16s:%s %15.3f %15.3f %15.3f\n",
	  ANSI_CYAN, "Coords X Y Z", ANSI_WHITE,
	  su2pc(sdb[x].coords.x - sdb[n].coords.xo),
	  su2pc(sdb[x].coords.y - sdb[n].coords.yo),
	  su2pc(sdb[x].coords.z - sdb[n].coords.zo));
...
char *format_s_Shield (int x)
{
	...
	for (i = 0 ; i < MAX_SHIELD_NAME ; ++i) {
		...
		d = sdb2dissipation(x, i); /* Anyasha - STDW - Shield Capacitor Updates */
		if (!sdb[x].shield.active[i]) {
			strncat(buffer, tprintf("CPM:%s",
			  unparse_power(d)), sizeof(buffer) - 1);
			strncat(buffer, tprintf(" Rat:%s",
			  unparse_power(sdb[x].shield.capacitor[i])), sizeof(buffer) - 1);
			strncat(buffer, tprintf("/%s\n",
			  unparse_power(sdb[x].shield.maximum * 40.0)), sizeof(buffer) - 1);
		} else {
			strncat(buffer, tprintf("%10.10s: %5.5s %s\n",
			  unparse_power(d), unparse_percent(sdb[x].shield.capacitor[i] / ( sdb[x].shield.maximum * 40.0 )),
			  ansi_stoplight_scale(sdb[x].shield.capacitor[i] / ( sdb[x].shield.maximum * 40.0 ), 20)), sizeof(buffer) - 1);
		}
	}
