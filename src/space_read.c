...
/* ENGINE */
	...
	result += convert_double(array[9], 0.0, &sdb[x].engine.warp_hard_cruise);
	result += convert_double(array[10], 0.0, &sdb[x].engine.impulse_hard_cruise);
	/* Last two lines added for hard cruise limits - Anyasha - STDW */
	if (result == 0) {
...
/* SHIELD */
    ...
    result += convert_double(array[16], 0.0, &sdb[x].shield.capacitor[0]);
    result += convert_double(array[17], 0.0, &sdb[x].shield.capacitor[1]);
    result += convert_double(array[18], 0.0, &sdb[x].shield.capacitor[2]);
    result += convert_double(array[19], 0.0, &sdb[x].shield.capacitor[3]);
    result += convert_double(array[20], 0.0, &sdb[x].shield.capacitor[4]);
    result += convert_double(array[21], 0.0, &sdb[x].shield.capacitor[5]);
	/* Added Shield Capacitors above - Anyasha - STDW */
    if (result == 0) {
