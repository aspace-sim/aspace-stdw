...
int do_set_fire (int first, int last, int weapon, int mode, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		notify(enactor, ansi_red(tprintf("%s is not in space.", Name(sdb[n].object))));
		return 0;
		...
		for (i = 0; i < sdb[n].beam.banks; ++i) {
			...
			if (sdb[x].tmp.i0 == n)
				if (dmg_b[i] > 0.0) {
					...
					/*	if (sdb[x].tmp.d1 > 0.0) { */
						if (sdb[x].shield.active[sdb[x].tmp.i3] && sdb[x].shield.capacitor[sdb[x].tmp.i3]) { /* Added for shield capacitors - Anya STDW */
...
/*								sdb[x].tmp.d0 += dmg_b[i] / sdb[x].tmp.d1 / 10.0;  */
								sdb[x].tmp.d0 += 1.0; /* Added so that d0 will increment for messaging purposes - Anya STDW */
								if (dmg_b[i] > sdb[x].shield.capacitor[sdb[x].tmp.i3]) { /* Following three if/else added for capacitors - Anya STDW */
									dmg_b[i] -= sdb[x].shield.capacitor[sdb[x].tmp.i3];
									sdb[x].shield.capacitor[sdb[x].tmp.i3] = 0;
								}
								else if (sdb[x].shield.capacitor[sdb[x].tmp.i3] > dmg_b[i]) {
									sdb[x].shield.capacitor[sdb[x].tmp.i3] -= (int)dmg_b[i];
									dmg_b[i] = 0.0;
								}
								else {
									dmg_b[i] = 0.0;
		...
		for (i = 0; i < sdb[n].missile.tubes; ++i) {
			...
			if (sdb[x].tmp.i0 == n)
				if (dmg_m[i] > 0.0) {
					if (sdb[x].shield.exist) {
					/*	if (sdb[x].tmp.d1 > 0.0) { */
						if (sdb[x].shield.active[sdb[x].tmp.i3] && sdb[x].shield.capacitor[sdb[x].tmp.i3]) { /* Added for capacitors - Anya STDW */
...
/*								sdb[x].tmp.d0 += dmg_m[i] / sdb[x].tmp.d1 / 10.0;	*/
								sdb[x].tmp.d0 += 1.0; /* Added so that d0 will increment for messaging purposes - Anya STDW */
								if (dmg_m[i] > sdb[x].shield.capacitor[sdb[x].tmp.i3]) { /* Following three if/else added for capacitors - Anya STDW */
									dmg_m[i] -= sdb[x].shield.capacitor[sdb[x].tmp.i3];
									sdb[x].shield.capacitor[sdb[x].tmp.i3] = 0;
								}
								else if (sdb[x].shield.capacitor[sdb[x].tmp.i3] > dmg_m[i]) {
									sdb[x].shield.capacitor[sdb[x].tmp.i3] -= (int)dmg_m[i];
									dmg_m[i] = 0.0;
								}
								else {
									dmg_m[i] = 0.0;
	...
	for (i = 0; i < sdb[n].sensor.contacts; ++i) {
		...
		if (sdb[x].tmp.i0 == n) {
			...
			sdb[x].tmp.d2 /= sdb[x].tech.armor; /* additive weapons */
			if (sdb[x].tmp.d0 > 0.0) {
/*				d_shield[sdb[x].tmp.i3] += sdb[x].tmp.d0;   **** Line removed for capacitors ****  */
				if ( (sdb[x].tmp.d2 < 0.0) && /* This statement added so that capacitor-only hits have a small chance of getting real damage - Anya STDW */
					( (get_random_long(0,10) + get_random_long(0,10) + 2) >= 16 ) )
					sdb[x].tmp.d2 = 1.0;
			}
			if (sdb[x].tmp.d2 > 0.0) { /* Moved this outside above if so that offline ships get 'ship hit' messages */
				alert_ship_hurt(x);
			} else
				alert_ship_hit(x);
			...
			if (sdb[x].tmp.d2 > 0.0) {
					if (mode == 0 || mode == 6) {
						d_system[0] += sdb[x].tmp.d2 / 10.0;
					} else
						d_system[0] += sdb[x].tmp.d2 / 25.0;
				...
				for (j = 0; j < k; ++j) {
					...
					if (mode == 0) /* normal damage */ {
					...
					} else if (mode == 1) /* shield damage added by Anya - STDW */ {
						switch (get_random_long(0,10) + get_random_long(0,10) + 2) {
							case 7: d_shield[get_random_long(0,MAX_SHIELD_NAME)] += pdmg; break;
							case 8: d_shield[get_random_long(0,MAX_SHIELD_NAME)] += pdmg; break;
							case 9: d_shield[get_random_long(0,MAX_SHIELD_NAME)] += pdmg; break;
							case 10: d_shield[get_random_long(0,MAX_SHIELD_NAME)] += pdmg; break;
							case 13: d_shield[get_random_long(0,MAX_SHIELD_NAME)] += pdmg; break;
							case 14: d_shield[get_random_long(0,MAX_SHIELD_NAME)] += pdmg; break;
							case 15: d_shield[get_random_long(0,MAX_SHIELD_NAME)] += pdmg; break;
							case 16: d_shield[get_random_long(0,MAX_SHIELD_NAME)] += pdmg; break;
							default: break;
						}
					} else if (mode == 2) /* engine damage */ {
						switch (get_random_long(0,10) + get_random_long(0,10) + 2) {
							case 11: d_system[14] += pdmg; break;
							case 12: d_system[6] += pdmg; break;
							case 13: d_system[14] += pdmg; break;
							case 14: d_system[6] += pdmg; break;
							case 15: d_system[14] += pdmg; break;
							case 16: d_system[6] += pdmg; break;
							default: break;
						}
						...
						switch (get_random_long(0,10) + get_random_long(0,10) + 2) {
							...
							case 8: d_missile[get_random_long(0,sdb[x].missile.tubes)] += pdmg; break;
							case 14: d_beam[get_random_long(0,sdb[x].beam.banks)] += pdmg; break;
							case 15: d_missile[get_random_long(0,sdb[x].missile.tubes)] += pdmg; break;
							case 16: d_beam[get_random_long(0,sdb[x].beam.banks)] += pdmg; break;
							case 17: d_system[13] += pdmg; break;
							case 18: d_system[12] += pdmg; break;
							case 19: d_system[4] += pdmg; break;
							default: break;
						...
						switch (get_random_long(0,10) + get_random_long(0,10) + 2) {
							...
							case 4: d_system[7] += pdmg; break;
							case 5: d_system[5] += pdmg; break;
							case 9: d_system[4] += pdmg; break;
							case 11: d_system[11] += pdmg; break;
							case 12: d_system[7] += pdmg; break;
							case 13: d_system[5] += pdmg; break;
							case 17: d_system[4] += pdmg; break;
							default: break;
						}
						...
						switch (get_random_long(0,10) + get_random_long(0,10) + 2) {
							case 6: d_system[1] += pdmg; break;
							case 7: d_system[8] += pdmg; break;
							case 9: d_system[2] += pdmg; break;
							case 12: d_system[1] += pdmg; break;
							case 14: d_system[8] += pdmg; break;
							case 18: d_system[2] += pdmg; break;
							default: break;
						}
					}
				}
...
int do_set_land (int contact, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		if (error_on_contact(n, x, enactor)) {
			...
			moveit(sdb[n].object, sdb[x].object, 1);
			if(sdb[x].structure.type == 3) {  /* Added for STDW - Anyasha for entering orbit of a planet.  If statement and first set of arguments added */
				do_console_notify(n, console_helm, console_tactical, console_science, ansi_cmd(enactor, tprintf("%s enters %s's atmosphere", Name(sdb[n].object), Name(sdb[x].object))));
				do_console_notify(x, console_helm, console_tactical, console_science, ansi_alert(tprintf("%s enters %s's atmosphere", Name(sdb[n].object), Name(sdb[x].object))));
				do_ship_notify(n, tprintf("The %s enters the atmosphere.", Name(sdb[n].object)));
				do_space_notify_two(n, x, console_helm, console_tactical, console_science, "enters the atmosphere of");
			} else {
...
int do_set_launch (dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		if (!x) {
			...
			moveit(sdb[n].object, Location(sdb[x].object), 1);
			if(sdb[x].structure.type == 3) { /* Added for STDW - Anyasha leaving planet orbit.  If and first set of arguments added */
				do_console_notify(n, console_helm, console_tactical, console_science, ansi_cmd(enactor, tprintf("%s leaves %s's atmosphere", Name(sdb[n].object), Name(sdb[x].object))));
				do_console_notify(x, console_helm, console_tactical, console_science, ansi_alert(tprintf("%s leaves %s's atmosphere", Name(sdb[n].object), Name(sdb[x].object))));
				do_ship_notify(n, tprintf("The %s leaves the atmosphere and returns to orbit.", Name(sdb[n].object)));
				do_space_notify_two(n, x, console_helm, console_tactical, console_science, "leaves the atmosphere of");
			} else {
				...
				do_ship_notify(n, tprintf("The %s launches from %s.", Name(sdb[n].object), Name(sdb[x].object)));
	...
	if (num) {
		...
		if (fix_cost > sdb[n].structure.repair) {
			*dmg += sdb[n].structure.repair / repair_mult[num] / mult / 100.0;
			...
			do_console_notify(n, console_engineering, 0, 0, /* Changed from damage to engineering - Anya STDW */
			  ansi_cmd(enactor, tprintf("%s repaired to %s",
			  system_name[num], unparse_percent(*dmg))));
			if (type)
				do_console_notify(x, console_engineering, 0, 0, /* Changed from damage to engineering - Anya STDW */
