...
/* beam ------------------------------------------------------------------- */
			case 'e': switch (f2[0]) {
				...
				/* case d added to get beam damage - Anya STDW */
				case 'd': sdb[x].blist.damage[parse_integer(f3) - 1] = parse_number(value); break;
...
/* missile ---------------------------------------------------------------- */
			case 'i': switch (f2[0]) {
				/* case d added to get miss damage via sdb - Anya STDW */
				case 'd': sdb[x].mlist.damage[parse_integer(f3) - 1] = parse_number(value); break;
...
/* shield ----------------------------------------------------------------- */
			case 'h': switch (f2[0]) {
				...
				case 'c': switch (f3[0]) { /* Anyasha - STDW - Shield Capacitors */
					case 'f': sdb[x].shield.capacitor[0] = parse_number(value); break;
					case 's': sdb[x].shield.capacitor[1] = parse_number(value); break;
					case 'a': sdb[x].shield.capacitor[2] = parse_number(value); break;
					case 'p': sdb[x].shield.capacitor[3] = parse_number(value); break;
					case 'd': sdb[x].shield.capacitor[4] = parse_number(value); break;
					case 'v': sdb[x].shield.capacitor[5] = parse_number(value); break;
					default: safe_str("#-1 NO SUCH FIELD", buff, bp); return; break;
					} break;
				case 'd': switch (f3[0]) {
