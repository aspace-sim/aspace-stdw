...
int do_operations_status (dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		strncat(buffer, "\n", sizeof(buffer) - 1);
		if (sdb[n].structure.type != 3) { /* If statement by Anya - STDW so planets don't show cargo cap etc. */
		...
		if (sdb[n].structure.has_docking_bay && sdb[n].structure.type != 3) { /* Anya - STDW so planets don't show landed and docked */
			strncat(buffer, format_l_line(), sizeof(buffer) - 1);
			strncat(buffer, format_l_Docked(n), sizeof(buffer) - 1);
		}
		if (sdb[n].structure.has_landing_pad && sdb[n].structure.type != 3) { /* Anya - STDW so planets don't show landed and docked */
			strncat(buffer, format_l_line(), sizeof(buffer) - 1);
...
int do_scanner_report (int a, char *s, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		switch (f) {
			...
			case 5:
				if (sdb[x].structure.type != 3) { /* Anya - STDW so planets don't show landed vessels or cargo etc on scans */
...
int do_planet_report (const char *s, int t, dbref enactor)
{
	...
	if (error_on_console(enactor)) {
		...
		if (c) {
			...
			for (i = 0; i < max; ++i) {
				if (pln[i])
					strncat(buffer, tprintf("%s%-19.19s%s %2.1s %7.7s %3.0f %-3.0f %7.7s %10.3f %10.3f %10.3f\n",
