...
void alert_shield_charged (int x, int i)  /* Anyasha - STDW - Shield Capacitors */
{
	do_console_notify(x, console_helm, console_engineering, 0,
	  ansi_alert(tprintf("%s capacitor fully charged", unparse_shield(i))));
	return;
}
...
void alert_gravity_well (int x) /* Anyasha - STDW - Gravity Wells */
{
	do_console_notify(x, console_helm, console_engineering, console_science,
		  ansi_cmd(sdb[x].object, "Warp drive disrupted by gravity interference"));
		  
	do_console_notify(x, console_helm, console_engineering, 0,
		  ansi_cmd(sdb[x].object, "Impulse drive engaged"));
