...
void up_autopilot (void)
{
	...
	if (sdb[n].move.out >= 1.0) /* Added by Anya STDW to make warp autopilot get only within 3000 SU */
		r -= 3000.0;
	if (r < 0.0)
		r = 0.0;
	if (r < 1.0) { 
		...
		if (sdb[n].move.out >= 1.0) {
			...
			do_console_notify(n, console_helm, 0, 0,
				ansi_cmd(sdb[n].object, "Autopilot approach initiated"));
		} else {
			s = 0; a = 0;
			do_console_notify(n, console_helm, 0, 0,
			  ansi_cmd(sdb[n].object, "Autopilot destination reached"));
			do_console_notify(n, console_helm, 0, 0,
			  ansi_cmd(sdb[n].object, "Autopilot disengaged"));
		}
	} else if (r < 10) { s = 0.03; a = 1;
	} else if (r < 50) { s = 0.10; a = 2;
	} else if (r < 100) { s = 0.30; a = 3;
	} else if (r < 300) { s = 0.50; a = 4;
	} else if (r < 500) { s = 0.65; a = 5;
	} else if (r < 1000) { s = 0.80; a = 6;
	} else if (r < 2000) { s = 0.99; a = 7;
	} else {
...
void up_sensor_list (void)
{
	...
	for (object = MIN_SPACE_OBJECTS ; object <= max_space_objects ; ++object)
		if (sdb[n].location == sdb[object].location &&
		  ...
		  n != object) {
			if (sdb[n].status.landed && sdb[sdb[n].location].structure.type == 3) {
				a1 = atr_get(sdb[n].object, "MANEUVER`LEVEL");
				a2 = atr_get(sdb[object].object, "MANEUVER`LEVEL");
				a3 = atr_get(sdb[n].object, "MANEUVER`AREA");
				a4 = atr_get(sdb[object].object, "MANEUVER`AREA");
				l1 = safe_atr_value(a1);
				l2 = safe_atr_value(a2);
				l3 = safe_atr_value(a3);
				l4 = safe_atr_value(a4);
				if (a1 == NULL || a2 == NULL || a3 == NULL || a4 == NULL) {
					continue;
				} else {
					if (parse_integer(l1) == 0) {
						if (parse_integer(l2) != 0) {
							continue;
						}
					} else if(parse_number(l3) != parse_number(l4)) {
						continue;
					}
				}
			}
...
void up_gravity_wells (void) /* Anyasha - STDW - Gravity Wells */
{
	register int object, i;
	double x, y, z, limit = 3000.0;
	for (object = MIN_SPACE_OBJECTS ; object <= max_space_objects ; ++object)
		{
		
		  if (sdb[n].location == sdb[object].location &&
		  sdb[n].space == sdb[object].space &&
		  ((sdb[object].structure.type == 5) || (sdb[object].structure.type == 3)) &&
		  n != object) {
			x = fabs(sdb[n].coords.x - sdb[object].coords.x);
			if ( x > limit )
			  	continue;
		    ...
		    if ((sdb[n].move.in >= 1.0)  ||  (sdb[n].move.in <= -1.0))
		      { 
			  if(sdb[n].move.in >= 1.0)
				{
				   if (sdb[n].engine.impulse_max >= sdb[n].engine.impulse_cruise)
				    {
				     sdb[n].move.in = sdb[n].engine.impulse_cruise;
		      		    }
				   else
				    {
				      sdb[n].move.in = sdb[n].engine.impulse_max;
...
void up_shieldcap (void) /* Anyasha - STDW - Shield Capacitors */
{
	register int i;
	
	for(i = 0; i < MAX_SHIELD_NAME; ++i)
	{
		if ( ( sdb[n].alloc.shield[i] * sdb[n].power.total ) && ( sdb[n].shield.damage[i] >= 0.01 ) && (sdb[n].shield.capacitor[i] < 
				(int)( sdb[n].shield.maximum * 40 * sdb[n].shield.damage[i] ) ) )
			{
				sdb[n].shield.capacitor[i] += ( sdb2dissipation(n, i) / 60.0 );
				
				if (sdb[n].shield.capacitor[i] >= (int)( sdb[n].shield.maximum * 40 * sdb[n].shield.damage[i] ) )
				
					alert_shield_charged(n, i);
			}
		if (sdb[n].shield.capacitor[i] >= 
			(int)( sdb[n].shield.maximum * 40 * sdb[n].shield.damage[i] ) )
			sdb[n].shield.capacitor[i] = (int)( sdb[n].shield.maximum * 40 * sdb[n].shield.damage[i] );
		if (sdb[n].shield.capacitor[i] < 0)
			sdb[n].shield.capacitor[i] = 0;
		...
		if (sdb[n].status.active && sdb[n].structure.type) {
				...
				if (sdb[n].move.dt > 0.0) {
					if (sdb[n].structure.type == 1)
						if (sdb[n].move.time - sdb[n].status.time > 18000) { /* Anyasha - STDW - Ship Idleout */
					...
					if (sdb[n].power.batt > 0.0 || sdb[n].alloc.miscellaneous > 0.0)
						up_reserve();
					if (sdb[n].alloc.shields) /* Anyasha - STDW - Shield Capacitors */
						up_shieldcap();
					...
					if (sdb[n].move.in != sdb[n].move.out) {
						up_gravity_wells(); /* Anyasha - STDW - Gravity Wells */
						up_speed_io();
						up_velocity();
						up_turn_rate();
					}
